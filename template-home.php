...
<article>
            ...
            <div class="md:px-44 md:py-10 p-5">
                <?php if (isset($hero_sec["program_heading"])) { ?>
                    <h1 class="text-center font-tnr mb-5">
                        <?php echo wp_kses_post($hero_sec["program_heading"]); ?>
                    </h1>
                <?php } ?>
                ...
                <div>
                    <?php if (isset($hero_sec["program_overview"]["heading"])) { ?>
                        <h2 class="mb-5 font-tnr">
                            <?php echo wp_kses_post($hero_sec["program_overview"]["heading"]); ?>
                        </h2>
                    <?php } ?>
                    <div>
    ...
    ?>
        <section class="md:px-44 md:py-10 p-5">
            <div>
                <?php echo wp_kses_post($paul_fisher_video_section["upper_description"]); ?>

                <h3 class="mb-5">
                    <?php echo wp_kses_post($paul_fisher_video_section["youtube_video"]["video_title"]); ?>
                </h3>
                ...
                <div class="w-full">
                    <div class="bg-gray-200 rounded-none">
                        <figure class="cursor-pointer relative group" onclick="lazyLoadVideo('<?php echo esc_attr($paul_fisher_video_section['youtube_video']['youtube_video_id']); ?>', this)">
                            ...
                            <button name="play video button" aria-label="Play video" class="play-btn" aria-label="Play video">
    ...
    <?php if ($key_highlights) : ?>
        <section style="background: url('<?php echo esc_url(wp_get_attachment_image_url($key_highlights["section_background_image"], "large")); ?>'); background-position: center; background-repeat: no-repeat; background-size: cover;" class="md:px-44 bg-center bg-no-repeat md:py-10 p-5 relative bg-primary text-white">
            <div>
                <h2 class="md:mb-20 mb-5 font-tnr">
                    <?php echo wp_kses_post($key_highlights["section_title"]); ?>
                </h2>
                    ...
                    <?php foreach ($key_highlights["key_highlight_points"] as $item) : ?>
                        <li class="flex items-center md:flex-1 flex-1 md:basis-1/3 gap-5">
                            <figure class="shrink-0 md:w-[80px] w-[60px]">
                                ...
                                <figcaption class="sr-only"><?php echo wp_kses_post(wp_get_attachment_caption($item["icon"])); ?></figcaption>
                            </figure>
    ...
    <?php if ($leadership_narratives) : ?>
        <section class="md:px-44 md:py-10 p-5">
            <div>
                <h2 class="mb-5 font-tnr">
                    <?php echo wp_kses_post($leadership_narratives["heading_1"]); ?>
                </h2>
                <hr class="mt-10 border-primary" />
            </div>
            <div>
                <h2 class="mb-5 font-tnr">
                    <?php echo wp_kses_post($leadership_narratives["section_title"]); ?>
                </h2>

                <h3 class="mb-3">
                    <?php echo wp_kses_post($leadership_narratives["pre_module"]["heading"]); ?>
                </h3>
                <p><?php echo wp_kses_post($leadership_narratives["pre_module"]["description"]); ?></p>

                <h2 class="mb-5 font-tnr">
                    <?php echo wp_kses_post($leadership_narratives["heading"]); ?>
                </h2>
                ...
                <?php foreach ($leadership_narratives["programme_modules"] as $index => $item) : ?>
                    ...
                    <div class="readmore-section md:mb-10 mb-5">
                        ...
                        <p><?php echo wp_kses_post($item["short_points"]); ?></p>

                        <div class="readmore-content mt-5" style="display: none;">
                            <?php echo wp_kses_post($item["read_more_description"]); ?>
                        </div>
    ...
    <?php if ($cohort_statistics) : ?>
        <section class="md:px-44 md:py-10 p-5">
            <h2 class="font-tnr md:text-center md:mb-10 mb-5"><?php echo wp_kses_post($cohort_statistics["section_title"]); ?></h2>
            <div class="flex md:flex-row flex-col justify-between md:gap-10 gap-5">
                <div class="basis-[40%]">
                    <h3 class="mb-3 text-center"><?php echo wp_kses_post($cohort_statistics["work_experience_chart"]["heading"]); ?></h3>
                    <figure>
                        <?php echo wp_get_attachment_image($cohort_statistics["work_experience_chart"]["chart_image"], "large", false, [
                            "loading" => "lazy",
                            "class" => "image-contain",
                        ]); ?>
                        <figcaption class="sr-only"><?php echo wp_kses_post(wp_get_attachment_caption($cohort_statistics["work_experience_chart"]["chart_image"])); ?></figcaption>
                    </figure>
                    <p class="my-3 text-center"><?php echo wp_kses_post($cohort_statistics["work_experience_chart"]["lower_text"]); ?></p>
                </div>
                <div>
                    <h3 class="mb-3"><?php echo wp_kses_post($cohort_statistics["designation"]["heading"]); ?></h3>
                    <?php echo wp_kses_post($cohort_statistics["designation"]["list"]); ?>
                </div>
                <div>
                    <h3 class="mb-3"><?php echo wp_kses_post($cohort_statistics["industry"]["heading"]); ?></h3>
                    <?php echo wp_kses_post($cohort_statistics["industry"]["list"]); ?>
                </div>
            </div>
            <h3 class="text-center md:my-10 my-5"><?php echo wp_kses_post($cohort_statistics["companies"]["heading"]); ?></h3>
            <figure class="md:mt-10 mt-5">
                <?php echo wp_get_attachment_image($cohort_statistics["companies"]["companies_image"], "large", false, [
                    "loading" => "lazy",
                    "class" => "image-cover",
                ]); ?>
                <figcaption class="sr-only"><?php echo wp_kses_post(wp_get_attachment_caption($cohort_statistics["companies"]["companies_image"])); ?></figcaption>
            </figure>
        ...
        <section class="md:py-10 py-5">
            <div class="md:px-44 px-5">
                <h2 class="font-tnr md:mb-10 mb-5">
                    <?php echo wp_kses_post($the_oxford_experience["heading"]); ?>
                </h2>
                <?php echo wp_kses_post($the_oxford_experience["description"]); ?>

                <br>

                <h3 class="md:mb-10 mb-5">
                    <?php echo wp_kses_post($the_oxford_experience["explore_the_oxford_campus"]["video_title"]); ?>
                </h3>
                <div class="bg-gray-200 rounded-none">
                    <figure class="cursor-pointer relative group" onclick="lazyLoadVideo('<?php echo esc_attr($the_oxford_experience['explore_the_oxford_campus']['youtube_video_id']); ?>', this)">
                        <?php echo wp_get_attachment_image($the_oxford_experience["explore_the_oxford_campus"]["video_thumbnail"], "large", false, [
                            "loading" => "lazy",
                            "class" => "image-video",
                        ]); ?>
                        <figcaption class="sr-only"><?php echo wp_kses_post(wp_get_attachment_caption($the_oxford_experience["explore_the_oxford_campus"]["video_thumbnail"])); ?></figcaption>
                ...
                <h3 class="md:my-10 my-5">
                    <?php echo wp_kses_post($the_oxford_experience["hear_from_paul_fisher_programme_director"]["video_title"]); ?>
                </h3>

                <div class="bg-gray-200 rounded-none">
                    <figure class="cursor-pointer relative group" onclick="lazyLoadVideo('<?php echo esc_attr($the_oxford_experience['hear_from_paul_fisher_programme_director']['youtube_video_id']); ?>', this)">
                        ...
                        <figcaption class="sr-only"><?php echo wp_kses_post(wp_get_attachment_caption($the_oxford_experience["hear_from_paul_fisher_programme_director"]["video_thumbnail"])); ?></figcaption>
    ...
    <?php if (!is_null($is_the_programme_right_for_me)) : ?>
        <section class="bg-primary text-white">
    ...
    <?php if (!is_null($application_process)) : ?>
        <section>
            <div class="md:px-44 md:py-10 p-5">
                <h2 class="font-tnr md:mb-10 mb-5">
                    <?php echo wp_kses_post($application_process["section_title"]); ?>
                </h2>
